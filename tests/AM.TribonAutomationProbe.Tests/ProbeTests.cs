...
public sealed class ProbeTests
{
    ...
    [Fact] public void AssistantCommandAndConfirmationOptionsParse() { var result = CliParser.Parse(["assistant", "--text=创建缺失的对象标签", "--confirm-write=true", "--allow-write=true"]); Assert.Null(result.Error); Assert.Equal("assistant", result.Options!.Command); Assert.Equal("创建缺失的对象标签", result.Options.Get("text")); Assert.True(result.Options.ConfirmWrite); Assert.True(result.Options.AllowWrite); }
