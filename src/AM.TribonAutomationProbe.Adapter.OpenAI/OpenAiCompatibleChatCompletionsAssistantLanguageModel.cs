...
public sealed class OpenAiCompatibleChatCompletionsAssistantLanguageModel : IAssistantLanguageModel
{
    private const int TimeoutMs = 60000, MaxTasks = 4, MaxUserTextCharacters = 8000, MaxHistoryTurns = 8, MaxHistoryCharacters = 12000;
    ...
    {
        if (string.IsNullOrWhiteSpace(context.UserText)) throw Error(ProbeErrorCodes.InvalidMessage, "Assistant input is empty.");
        if (context.UserText.Length > MaxUserTextCharacters) throw Error(ProbeErrorCodes.InvalidMessage, "Assistant input is too long.");
        var messages = new List<object> { new { role = "system", content = SystemPrompt } };
        foreach (var turn in (context.History ?? Array.Empty<AssistantConversationTurn>()).TakeLast(MaxHistoryTurns)) { if (!string.IsNullOrWhiteSpace(turn.Content)) messages.Add(new { role = turn.Role == "assistant" ? "assistant" : "user", content = turn.Content[..Math.Min(turn.Content.Length, MaxHistoryCharacters)] }); }
    ...
    private AssistantInterpretation Parse(string body, string? requestId, long latency, string userText)
    {
        try
        {
            using var doc = JsonDocument.Parse(body); var root = doc.RootElement; var choices = root.GetProperty("choices"); if (choices.GetArrayLength() == 0) throw Error(ProbeErrorCodes.AssistantModelInvalidResponse, "choices is empty."); var contentElement = choices[0].GetProperty("message").GetProperty("content"); if (contentElement.ValueKind != JsonValueKind.String) throw Error(ProbeErrorCodes.AssistantModelInvalidResponse, "message.content must be a string."); var content = contentElement.GetString(); if (string.IsNullOrWhiteSpace(content)) throw Error(ProbeErrorCodes.AssistantModelInvalidResponse, "message.content is empty."); content = Unwrap(content);
            using var interpretation = JsonDocument.Parse(content); var value = interpretation.RootElement; if (value.ValueKind != JsonValueKind.Object) throw Error(ProbeErrorCodes.AssistantModelInvalidResponse, "Assistant content must be one JSON object.");
    ...
    private static string SafeDetail(string message) => message.Length <= 500 ? message : message[..500];
    private sealed class Wire { public required List<Item> Tasks { get; init; } public required bool ClarificationRequired { get; init; } public string? ClarificationQuestion { get; init; } public string? Explanation { get; init; } }
    private sealed class Item { public required string Intent { get; init; } public required double Confidence { get; init; } }
}
